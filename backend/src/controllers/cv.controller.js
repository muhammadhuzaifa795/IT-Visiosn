...
import CvGenerator from "../models/CvGenerator.js"
import { generateAICV } from "../services/cv.service.js";
...
export async function getCV(req, res) {
  try {
    ...
    if (!cvs || cvs.length === 0) {
      // Agar koi CV nahi milta, toh 200 OK ke saath empty array return karein
      // Taki frontend par 404 error na aaye aur "No CVs Found" message dikhe
...
export const updateCV = async (req, res) => {
  try {
    ...
    // Authorization check: Ensure the authenticated user owns this CV
    if (req.user._id.toString() !== cv.userId.toString()) {
...
//     // Authorization check: Ensure the authenticated user owns this CV
//     if (req.user._id.toString() !== cv.userId) {
...
export const deleteCV = async (req, res) => {
  try {
    ...
    // ✅ FIXED: Compare string vs string
    if (req.user._id.toString() !== cv.userId.toString()) {
      return res.status(403).json({ error: "Unauthorized: You are not authorized to delete this CV." });
    }
