...
const Leaderboard = () => {
  ...
  const processedLeaderboard = leaderboard.map((user, index) => ({
    ...
    specialty: mockSpecialties[index % mockSpecialties.length], // Mocked
    level: mockLevels[Math.min(index, mockLevels.length - 1)], // Mocked
  }));
  ...
  if (fetchError || processedLeaderboard.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-base-100 via-base-200/50 to-base-300/30 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-base-content mb-2">No Users Available</h2>
          <p className="text-base-content/60">
            {fetchError ? fetchError.message : "No leaderboard data available at this time."}
          </p>
        </div>
      </div>
    );
                  ...
                  <div className="bg-base-100/80 backdrop-blur-md rounded-2xl p-4 border border-base-300/50 shadow-2xl mb-4 w-44 md:w-52 text-center">
                    ...
                    <div className="flex items-center justify-center gap-4 text-xs text-base-content/60">
                      <div className="flex items-center gap-1">
                        ...
                        <span>{topThree[0].ticketsSolved} tickets</span>
                      </div>
                    </div>
                  </div>
                  <div
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-medium ${
                              participant.level === "Advanced" || participant.level === "Expert"
                          ...
                          <span className="px-2 py-1 rounded-full text-xs font-medium bg-accent/20 text-accent">
                            {participant.ticketsSolved} Tickets
                          </span>
                        </div>
                      </div>
                    </div>
